  ...
  >
    ...
    <v-col class="col-12 col-md-6 col">
      ...
      <v-card elevation="12" class="mx-1" style="background: #344e5e; color: white;">
        <v-spacer></v-spacer>
        <v-card-title>
          <div class="display-1 my-2">Modifica tus datos</div>
        </v-card-title>
        <v-spacer></v-spacer>
        <v-card-text>
          <v-form ref="form" v-model="valid" :lazy-validation="lazy">
            ...
            <div class="form-group row ma-0 pa-0">
              <div class="col-sm-12 mb-sm-0 ma-0 pa-0">
                <v-checkbox
                  style="color: white;"
                  @change="chkPassChanged"
                  color="white"
                  v-model="checkbox1"
                  label="Cambiar password"
                ></v-checkbox>
              </div>
              <div class="col-sm-6 mb-sm-0" v-if="changePass">
                <v-text-field
                  v-model="pass"
                  :counter="10"
                  :rules="passRules"
              ...
              <div class="col-sm-6" v-if="changePass">
                <v-text-field
                  v-model="passRep"
                  :counter="10"
                  :rules="passRepRules"
...
export default {
  ...
  created: function () {
    ...
    this.getUser().then(function (result) {
      ctx.email = result.respuesta.email;
      ctx.nombre = result.respuesta.firstName;
      ctx.apellidos = result.respuesta.lastName;
      ctx.filePreview = result.respuesta.imageBase64;
      ctx.imagen = result.respuesta.imageBase64;
    });
  },
  data: () => ({
    show1: false,
  ...
  methods: {
    chkPassChanged(e) {
      this.changePass = JSON.parse(e);
    },
    getUser() {
      return new Promise(function (resolve, reject) {
        $.ajax({
          type: "POST",
          url: "http://127.0.0.1:8090/users/getUser",
          data: JSON.stringify({
            tokata: sessionStorage.getItem("token"),
          }),
          headers: {
    ...
    checkRegistro() {
      ...
      $.ajax({
        type: "POST",
        url: "http://127.0.0.1:8090/users/modificaUser",
        ...
        success: function (result) {
          if (
            ...
            alertas.error(
              ...
              "Por favor, revisa los campos introducidos."
            );
          } else {
